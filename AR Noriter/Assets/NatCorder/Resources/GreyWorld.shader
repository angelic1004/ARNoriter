...
Shader "Hidden/NatCorder/GreyWorld" {
	Properties {
		_MainTex ("Texture", 2D) = "white" {}
		_Rotation ("Rotation", float) = 0
		_Scale ("Scale", float) = 1
		_Greyness ("Greyness", float) = 0
	}
	SubShader {
		...
		Pass {
			...
			fixed4 frag (v2f i) : SV_Target {
				...
				fixed grey = dot(col.rgb, fixed3(0.2126, 0.7152, 0.0722));
				return lerp(col, fixed4(grey, grey, grey, 1.0), _Greyness);
			}
			ENDCG
		}
	}
